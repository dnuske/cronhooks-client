...
export default function Main() {
  ...
  const { isLoading, data: cronhooks } = useQuery(['cronhooks'], () => getAllHooks(accessToken), {refetchInterval: 10000})
  ...
  if (isLoading) {
    return <Center style={{marginTop: 300, fontSize: 50}}>
      <Loader />
    </Center>
  }
  if (cronhooks.length > 0) {
    ...
    return <Center style={{marginTop: 300, fontSize: 50}}>
      {os === 'macos' ? <><Kbd style={{fontSize: 50}}>⌘</Kbd> + <Kbd style={{fontSize: 50}}>k</Kbd></> : <><Kbd
        style={{fontSize: 50}}>ctrl</Kbd> + <Kbd style={{fontSize: 50}}>k</Kbd></>}&nbsp;to get started
    </Center>
  }
